...
def p_statement_CR(p):
    ...
    if p[1] not in mcc.INSTR_TYPE_CR1 and (p[1] not in mcc.INSTR_TYPE_CR2):
        cp.cprint_fail("Error:" + str(p.lineno(1)) +
                       ": Incorrect opcode or arguments compressed register")
        raise SyntaxError
    p[0] = {
        'opcode': p[1],
        'rd': p[2],
...
def p_statement_I_S_SB(p):
    ...
    if (p[1] not in mcc.INSTR_TYPE_I) and (p[1] not in mcc.INSTR_TYPE_S) and (p[1] not in mcc.INSTR_TYPE_SB) and (p[1] not in mcc.INSTR_TYPE_CLS):
        cp.cprint_fail("Error:" + str(p.lineno(1)) +
                       ": Incorrect opcode or arguments")
        raise SyntaxError
    #compressed load store
    elif p[1] in mcc.INSTR_TYPE_CLS:
        ret, imm, msg = get_imm_CLS(p[6], p.lineno(6))
        if not ret:
            cp.cprint_fail("Error:" + str(p.lineno(6)) + ":" + msg +"CLS")
            raise SyntaxError
        p[0] = {
            'opcode': p[1],
            'rs1': p[4],
...
def p_statement_U_UJ_CI(p):
    'statement : OPCODE register COMMA IMMEDIATE NEWLINE'

    if (p[1] not in mcc.INSTR_TYPE_U) and (p[1] not in mcc.INSTR_TYPE_UJ) and (p[1] not in mcc.INSTR_TYPE_CI1) and (p[1] not in mcc.INSTR_TYPE_CI2):
        cp.cprint_fail("Error:" + str(p.lineno(1)) +
                       ": Incorrect opcode or arguments")
        raise SyntaxError
        ...
        ret, imm, msg = get_imm_CI(p[4], p.lineno(4))
        if not ret:
            cp.cprint_fail("Error:" + str(p.lineno(4)) + ":" + msg)
            raise SyntaxError
...
def get_imm_CI(imm10, lineno):
    try:
        ...
        return False, imm10, msg
    '''
    CI instructions with immediate value split into 2 fields
    '''
    IMM_MAX = 0b011111
    IMM_MIN = -0b100000
    ...
    imm2 = format(imm10 if imm10 >= 0 else (1 << 7) + imm10, '06b')
    # imm2 = imm2[0:-1]
    imm2_6b = imm2[1:] if imm10 <0 else imm2
    assert(len(imm2_6b) == 6)
    imm_5 = imm2_6b[0]
    imm_4_0 = imm2_6b[1:]
    return True, (imm_5, imm_4_0), p_statement_none

def get_imm_CLS(imm10, lineno):
    try:
        imm10 = int(imm10)
    except:
        msg = "Invalid immediate specified."
    ...
    imm2 = format(imm10 if imm10 >= 0 else (1 << 6) + imm10, '05b')
    # imm2 = imm2[0:-1]

 #   assert(len(imm2) == 5)
    imm_3_5 = imm2[1:4]
    imm_5_3 = imm_3_5[2] + imm_3_5[1] + imm_3_5[0]
    imm_2_6 = imm2[4] + imm2[0]
    return True, (imm_5_3, imm_2_6), p_statement_none
